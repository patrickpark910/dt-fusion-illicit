...
# kg fertile isotope per m3 of nominal breeder volume.
LOADINGS = [0.0, 0.01, 0.10, 0.50, 1, 10, 25, 50, 75, 100, 150, 250, 500, 750, 999.99, 2000, 3000, 4000, 5000, 10000] # FERTILE_KGM3  # from Python/parameters.py
...
def make_flibe(fertile_kgm3, fertile_isotope='U238'):
    ...
    if fertile_isotope == 'U238':
        fertile = openmc.Material(name='UF4', temperature=TEMP_K)
        fertile.add_elements_from_formula('UF4', 'ao', enrichment=ENRICH_U)
        fertile.set_density('g/cm3', DENSITY_UF4)
        xf4_x_ratio = AMU_UF4 / ((100 - ENRICH_U) / 100 * AMU_U238)
    elif fertile_isotope == 'Th232':
        fertile = openmc.Material(name='ThF4', temperature=TEMP_K)
        fertile.add_elements_from_formula('ThF4', 'ao')
        fertile.set_density('g/cm3', DENSITY_ThF4)
        xf4_x_ratio = AMU_ThF4 / AMU_Th232

    vf_xf4 = fertile_kgm3 * xf4_x_ratio / (fertile.density * 1000.0)
    vf_flibe = 1.0 - vf_xf4

    if vf_xf4 == 0.0:
        return flibe

    mix = openmc.Material.mix_materials([flibe, fertile], [vf_flibe, vf_xf4], "vo")
...
def make_dcll(fertile_kgm3, fertile_isotope='U238'):
    ...
    if fertile_kgm3 == 0.0:
        ...
        breeder_mix = openmc.Material.mix_materials([biso, pbli], [vf_biso, vf_breeder], 'vo')
        mix = openmc.Material.mix_materials([breeder_mix, f82h, sic, he], [DCLL_VF_LL_NOM, DCLL_VF_FS_NOM, DCLL_VF_SI_NOM, DCLL_VF_HE_NOM], 'vo')
    mix.temperature = TEMP_K
    return mix
...
def make_hcpb(fertile_kgm3, fertile_isotope='U238'):
    ...
    vf_biso, vf_breeder, _ = calc_biso_vol_fracs(
        fertile_kgm3, fertile_isotope=fertile_isotope)
    ...
    if fertile_kgm3 == 0.0:
        ...
        breeder_mix = openmc.Material.mix_materials(
            [biso, li4sio4, be_mat],
            [vf_biso, vf_breeder * HCPB_VF_LI_IN_BREEDER_NOM, vf_breeder * HCPB_VF_BE_IN_BREEDER_NOM],
            'vo')
        mix = openmc.Material.mix_materials(
            [breeder_mix, eurofer, he],
            [HCPB_VF_BREEDER_NOM, HCPB_VF_EU_NOM, HCPB_VF_HE_NOM],
            'vo')
    mix.temperature = TEMP_K
    return mix
...
def main():
    ...
    flibe_u_nrm = read_cached_rows(OUT_CSV_FLIBE_U_NRM) or run_nrm("FLiBe (UF4)", lambda l: make_flibe(l, 'U238'), LOADINGS, OUT_CSV_FLIBE_U_NRM, flux_csv=CSV_FLUX_FLIBE_U, fertile='U238', fissile='U235')
    ...
    flibe_th_nrm = read_cached_rows(OUT_CSV_FLIBE_TH_NRM) or run_nrm("FLiBe (ThF4)", lambda l: make_flibe(l, 'Th232'), LOADINGS, OUT_CSV_FLIBE_TH_NRM, flux_csv=CSV_FLUX_FLIBE_TH, fertile='Th232', fissile=None)
    hcpb_th_nrm  = read_cached_rows(OUT_CSV_HCPB_TH_NRM)  or run_nrm("HCPB (ThO2-SiC)", lambda l: make_hcpb(l, 'Th232'), LOADINGS, OUT_CSV_HCPB_TH_NRM, flux_csv=CSV_FLUX_HCPB_TH, fertile='Th232', fissile=None)
    dcll_th_nrm  = read_cached_rows(OUT_CSV_DCLL_TH_NRM)  or run_nrm("DCLL (ThO2-SiC)", lambda l: make_dcll(l, 'Th232'), LOADINGS, OUT_CSV_DCLL_TH_NRM, flux_csv=CSV_FLUX_DCLL_TH, fertile='Th232', fissile=None)
