...
def rename_files(directory="."):
    ...
    for filename in os.listdir(directory):
        ...
        # Check if it is a FILE (not a directory) and contains our search string
        if os.path.isfile(old_path) and search_str in filename:
            new_name = filename.replace(search_str, replace_str)
            new_path = os.path.join(directory, new_name)
            
            # Check if the target filename already exists to prevent overwriting
