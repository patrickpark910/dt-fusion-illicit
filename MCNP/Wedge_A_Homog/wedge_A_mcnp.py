...
def main(fertile_kgm3, go_pack=False, go_print=False):
    ...
    **Be careful NOT to renormalize everything. DEDUCT the volume fraction of BISO from 1, and then
    ...
    V1_br = V1 # A1*h1 - N1 * BISO_VOLUME
    V2_br = V2 # A2*h2 - N2 * BISO_VOLUME
    V_br  = V1_br + V2_br
    ...
    """
    Breeder material card
    """
    # Dictionary of homogenized breeding region densities from OpenMC
    rho_breeder, mat_breeder = convert_openmc_to_mcnp(f"./mat_cards/{density_str}.txt", mat_id="3", suffix=".02c")
    print(rho_breeder, mat_breeder)
    ...
    """
    if go_print:
        with open(f'wedge_{case}.template', 'r') as f:
