...
def main():
    ...
    current_run = RPBLI(enrich_li=90, u_list=N_TRISO_PBLI)
...
class RPBLI:
    ...
    def __init__(self, enrich_li=90, u_list=N_TRISO_PBLI):
        ...
        mix_list = []
        for n in self.u_list:
            trisos =  n * VOL_ROB
            vf_triso = (vol_tot * trisos) / VOL_ROB
            remaining = 1.0 - vf_triso

            vf_pbli = 0.805 
            vf_he = 0.097 
            vf_f82h = 0.019 
            vf_sic = 0.079  
            sum_rest = vf_pbli + vf_he + vf_f82h + vf_sic
            vf_pbli = vf_pbli / sum_rest * remaining
            vf_he   = vf_he   / sum_rest * remaining
            vf_f82h = vf_f82h / sum_rest * remaining
            vf_sic  = vf_sic  / sum_rest * remaining

            # Now mix TRISO + other mix:
            mix = openmc.Material.mix_materials(
                [triso, pbli, he, f82h, SiC],
                [vf_triso, vf_pbli, vf_he, vf_f82h, vf_sic],
                'vo'
            )
            ...
            print(f"\nFraction of TRISO volume in blanket: {vf_triso}")
            print(f"\nFraction of pb: {vf_pbli}")
