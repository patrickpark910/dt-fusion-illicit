...
FERTILE_KGM3_FAST = [0.10, 1.5, 15.0, 150.0, 500.0]
FERTILE_KGM3_FULL = [
    0.10,
    0.50,
    1.5,
...
class Prism():
    ...
    def tallies(self, debug=False):
        ...
        # Multiple cells → one tally bin per cell per energy; sum bins for total breeding-blanket spectrum (CSV).
        ...
        # Flux (integral + spectrum) in breeding blanket only — matches A/C comparison CSVs
    ...
    def prism_helpers(self, debug=False):
        ...
        # Fast defaults: override with OPENMC_PARTICLES / OPENMC_BATCHES (e.g. 10000 and 20 for production)
        self.nps = int(os.environ.get("OPENMC_PARTICLES", "4000"))
        self.batches = int(os.environ.get("OPENMC_BATCHES", "8"))
    ...
    def openmc(self, debug=False, write=True, run=False, tracks=False):
        ...
        if run:
            if has_statepoint(self.path):
                ...
                self.model.run(cwd=self.path, tracks=tracks)


def _run_prism_job(args):
    """One OpenMC case for ProcessPoolExecutor (set OMP_NUM_THREADS=1 per process to avoid oversubscription)."""
    iso, case, fertile_kgm3, debug = args
    os.environ.setdefault("OMP_NUM_THREADS", "1")
    openmc.reset_auto_ids()
    run = Prism(case, fertile_kgm3, isotope=iso)
    run.openmc(debug=debug, write=True, run=True, tracks=False)
    return run.name
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--sequential",
        action="store_true",
        help="Run one case after another (no multiprocessing).",
    )
    parser.add_argument(
        "--workers",
        type=int,
        default=None,
        help="Parallel OpenMC processes (default: min(8, CPU count)).",
    )
    parser.add_argument(
        "--full-grid",
        action="store_true",
        dest="full_grid",
        help="Use full fertile kg/m³ list (many more cases).",
    )
    parser.add_argument(
        "--debug",
        action="store_true",
        help="Verbose prints in materials/geometry.",
    )
    args = parser.parse_args()

    os.makedirs("./OpenMC/", exist_ok=True)
    fertile_list = FERTILE_KGM3_FULL if args.full_grid else FERTILE_KGM3_FAST
    jobs = [
        (iso, case, fk, args.debug)
        for iso in ("U238", "Th232")
        for case in ("A", "C")
        for fk in fertile_list
    ]

    if args.sequential:
        for job in jobs:
            name = _run_prism_job(job)
            print(f"Done: {name}")
    else:
        n_workers = args.workers if args.workers is not None else min(8, os.cpu_count() or 4)
        n_workers = max(1, n_workers)
        print(f"Running {len(jobs)} cases with {n_workers} workers (OMP_NUM_THREADS=1 per process).")
        with ProcessPoolExecutor(max_workers=n_workers) as ex:
            for name in ex.map(_run_prism_job, jobs):
                print(f"Done: {name}")