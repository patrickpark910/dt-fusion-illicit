...
class Plot:
    ...
    def __init__(self, save=False, show=True):
        ...
        tbr_factor_path    = './Figures/Data/wedge_HCPB_TBR_factor_900K.csv'
        fissile_factor_path = './Figures/Data/wedge_HCPB_fissile_factor_900K.csv'
        if os.path.isfile(tbr_factor_path):
            tbr_fac = pd.read_csv(tbr_factor_path)
            for iso, rr_df_attr, iso_key in [('U238', 'hcpb_u_rr_df', 'U238'), ('Th232', 'hcpb_th_rr_df', 'Th232')]:
                fac = tbr_fac[tbr_fac['isotope'] == iso][['fertile_kg/m3', 'factor']]
                df = getattr(self, rr_df_attr)
                df = df.merge(fac, on='fertile_kg/m3', how='left')
                df['tbr'] = df['tbr'] * df['factor'].fillna(1.0)
                df = df.drop(columns=['factor'])
                setattr(self, rr_df_attr, df)
        if os.path.isfile(fissile_factor_path):
            fis_fac = pd.read_csv(fissile_factor_path)
            for iso, rr_df_attr, col in [('U238', 'hcpb_u_rr_df', 'Pu239_kg/yr'), ('Th232', 'hcpb_th_rr_df', 'U233_kg/yr')]:
                fac = fis_fac[fis_fac['isotope'] == iso][['fertile_kg/m3', 'factor']]
                df = getattr(self, rr_df_attr)
                df = df.merge(fac, on='fertile_kg/m3', how='left')
                df[col] = df[col] * df['factor'].fillna(1.0)
                df = df.drop(columns=['factor'])
                setattr(self, rr_df_attr, df)
            for iso, ng_df_attr in [('U238', 'hcpb_u_ng_df'), ('Th232', 'hcpb_th_ng_df')]:
                fac = fis_fac[fis_fac['isotope'] == iso][['fertile_kg/m3', 'factor']]
                df = getattr(self, ng_df_attr)
                df = df.merge(fac, on='fertile_kg/m3', how='left')
                df['mean'] = df['mean'] * df['factor'].fillna(1.0)
                df = df.drop(columns=['factor'])
                setattr(self, ng_df_attr, df)
    ...
    def _strictly_increasing_xy(self, x, y):
        """Return (x, y) sorted by x with duplicate x removed so x is strictly increasing (Akima needs this)."""
        x, y = np.asarray(x, dtype=float), np.asarray(y, dtype=float)
        order = np.argsort(x)
        x_sorted, y_sorted = x[order], y[order]
        keep = np.concatenate([[True], x_sorted[1:] > x_sorted[:-1]])
    ...
    def plot_tbr(self):
        ...
        # HCPB: use factor-adjusted 'tbr' column from rxns CSV (scaled in __init__)
        x6, y6 =   self.hcpb_u_rr_df['fertile_kg/m3'].values, BLANKET_COVERAGE * self.hcpb_u_rr_df['tbr'].values
        x5, y5 =  self.hcpb_th_rr_df['fertile_kg/m3'].values, BLANKET_COVERAGE * self.hcpb_th_rr_df['tbr'].values
        x4, y4 =   self.dcll_u_rr_df['fertile_kg/m3'].values, BLANKET_COVERAGE * (  self.dcll_u_rr_df['Li6(n,t)'] +   self.dcll_u_rr_df['Li7(n,Xt)'] ).values
        x3, y3 =  self.dcll_th_rr_df['fertile_kg/m3'].values, BLANKET_COVERAGE * ( self.dcll_th_rr_df['Li6(n,t)'] +  self.dcll_th_rr_df['Li7(n,Xt)'] ).values
        x2, y2 =  self.flibe_u_rr_df['fertile_kg/m3'].values, BLANKET_COVERAGE * ( self.flibe_u_rr_df['Li6(n,t)'] +  self.flibe_u_rr_df['Li7(n,Xt)'] ).values
        x1, y1 = self.flibe_th_rr_df['fertile_kg/m3'].values, BLANKET_COVERAGE * (self.flibe_th_rr_df['Li6(n,t)'] + self.flibe_th_rr_df['Li7(n,Xt)'] ).values

        # Ensure strictly increasing x (required by Akima1DInterpolator); dedupe by keeping first y per x
        x1, y1 = self._strictly_increasing_xy(x1, y1)
        x2, y2 = self._strictly_increasing_xy(x2, y2)
        x3, y3 = self._strictly_increasing_xy(x3, y3)
        x4, y4 = self._strictly_increasing_xy(x4, y4)
        x5, y5 = self._strictly_increasing_xy(x5, y5)
        x6, y6 = self._strictly_increasing_xy(x6, y6)
        ...
        if self.save:
            plt.savefig(f'./Figures/pdf/fig_tbr_1000kgm3_adjusted.pdf', bbox_inches='tight', format='pdf')
            plt.savefig(f'./Figures/png/fig_tbr_1000kgm3_adjusted.png', bbox_inches='tight', format='png')
    ...
    def plot_pu_per_yr(self):
        ...
        # Setting up x, y separately here so you can remove the impurity/wppm-magnitude cases --ppark 2025-08-06
        # HCPB: use factor-adjusted fissile kg/yr columns from rxns CSV (scaled in __init__)
        x6, y6 =   self.hcpb_u_rr_df['fertile_kg/m3'].values, BLANKET_COVERAGE * self.hcpb_u_rr_df['Pu239_kg/yr'].values
        x5, y5 =  self.hcpb_th_rr_df['fertile_kg/m3'].values, BLANKET_COVERAGE * self.hcpb_th_rr_df['U233_kg/yr'].values
        x4, y4 =   self.dcll_u_rr_df['fertile_kg/m3'].values, BLANKET_COVERAGE * pu239_at_to_kg_per_yr *   self.dcll_u_rr_df['U238(n,g)'].values
        x3, y3 =  self.dcll_th_rr_df['fertile_kg/m3'].values, BLANKET_COVERAGE *  u233_at_to_kg_per_yr *  self.dcll_th_rr_df['Th232(n,g)'].values
        x2, y2 =  self.flibe_u_rr_df['fertile_kg/m3'].values, BLANKET_COVERAGE * pu239_at_to_kg_per_yr *  self.flibe_u_rr_df['U238(n,g)'].values
        x1, y1 = self.flibe_th_rr_df['fertile_kg/m3'].values, BLANKET_COVERAGE *  u233_at_to_kg_per_yr * self.flibe_th_rr_df['Th232(n,g)'].values

        # Ensure strictly increasing x (required by Akima1DInterpolator)
        x1, y1 = self._strictly_increasing_xy(x1, y1)
        x2, y2 = self._strictly_increasing_xy(x2, y2)
        x3, y3 = self._strictly_increasing_xy(x3, y3)
        x4, y4 = self._strictly_increasing_xy(x4, y4)
        ...
        if self.save:
            plt.savefig(f'./Figures/pdf/fig_fissile_per_yr_1000kgm3_adjusted.pdf', bbox_inches='tight', format='pdf')
            plt.savefig(f'./Figures/png/fig_fissile_per_yr_1000kgm3_adjusted.png', bbox_inches='tight', format='png')
    ...
    def plot_cum_norm_histogram(self):
        ...
        if self.save:
            plt.savefig(f'./Figures/pdf/fig_cum_norm_histogram_adjusted.pdf', bbox_inches='tight', format='pdf')
            plt.savefig(f'./Figures/png/fig_cum_norm_histogram_adjusted.png', bbox_inches='tight', format='png')
