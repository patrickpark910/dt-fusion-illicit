...
class PlotStatepointPbLi:
    ...
    def __init__(self, enrich_li=90, u_list=MASS_U_LIST_PBLI, save=False, show=True, to_csv=False):
        ...
        self.name = f'DCLL_FW4cm_Li{self.e}_900K_2025-07-22'
    ...
    def print_tallies(self):
        ...
        """
        df = pd.DataFrame({'MT_fertile':MASS_U_LIST_PBLI,
    ...
    def plot_pu(self):
        ...
        plt.title(f'Pu-239 production rate (PbLi, {self.e}wt% Li-6)')
        plt.xlabel('Uranium loaded [metric tons]')
    ...
    def plot_pu_per_mtu(self):
        ...
        Pu_per_mtu_list = []
        for i, m in enumerate(self.fertile_mt_list[1:11]):
            Pu_per_mtu_list.append(self.U238_ng_list[i+1]/m) 

        plt.figure()
        plt.plot(self.fertile_mt_list[1:11], Pu_per_mtu_list, markersize=2, linewidth=0.75, color='#000000',) # turn capsize > 0 to show error bars, but they're super smol
    ...
    def plot_rxn_rates(self):
        ...
        for i, cell_id in enumerate(self.cell_ids):
            ...
            plt.title(f'Reaction rates, {self.fertile_mt_list[i]} MTU (PbLi, {self.e}wt% Li-6, log-log)')
            ...
            plt.title(f'Reaction rates, {self.fertile_mt_list[i]} MTU (PbLi, {self.e}wt% Li-6, lin-log)')
    ...
    def plot_pu_vs_energy(self):
        ...
        plt.title(f'U-238 (n,gamma) rxn rate (PbLi, {self.e}wt% Li-6)')
        plt.xscale('log'), plt.yscale('linear')
        ...
        if self.show: plt.show()
        plt.close('all')
        '''MUST FID THIS SO I CAN COMPARE WITH 1MTU bring back 1mtu'''
    ...
    def plot_rel_pu_vs_energy(self):
        ...
        for i, cell_id in enumerate(self.cell_ids):
            if self.fertile_mt_list[i] in [0.5]:
