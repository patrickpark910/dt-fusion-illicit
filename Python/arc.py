...
class ARC(Reactor):
    ...
    def initialize(self):
        ...
        self.temp_k          = TEMP_K
        self.blanket_name    = 'ARC'
        self.blanket_volume  = ARC_BL_VOL    # m³
        ...
        self.name = f"{self.run_type}_{self.blanket_name}_{self.temp_k}K_Li{self.breeder_enrich:04.1f}_{self.fertile_isotope}_{self.fertile_kgm3:07.2f}kgm3"         
        self.path = f"./OpenMC/{self.name}"
        
        os.makedirs(self.path, exist_ok=True)

        start_msg = f"\n======== {self.blanket_name} reactor - {self.fertile_isotope} {self.fertile_kgm3:07.2f} kg/m3 - {self.breeder_enrich:4.1f}%-enriched Li - {self.temp_k} K ========"
        print(f"{C.BLUE}{start_msg}{C.END}")
