...
fertile_bulk_density_kgm3 = 0.03
fertile_element = 'U'
breeder_volume = (42+85)*50*50/100**3
...
breeder = openmc.Material(name='breeder', temperature=TEMP_K)
...
vol_li4sio4_be = breeder_volume*(vf_li4sio4+vf_be) # for original 
vf_li4sio4_be, vf_biso = calc_biso_blanket_vol_fracs(fertile_bulk_density_kgm3, vol_li4sio4_be, fertile_element=fertile_element)
print(f"packing fraction {vf_biso}")
# biso displace the lithium orthosilicate and beryllium pebbles
# vf_biso = packing fraction in the blanket 
...
else: # heterogeneous case - don't have to worry about changing volume fractions 
    heterogeneous_blanket = openmc.Material.mix_materials([li4sio4, be, eurofer, he], [vf_li4sio4, vf_be, vf_eurofer, vf_he], 'vo') # breeder - still original vol frac
...
n = 50 # square length
xmin, xmax = openmc.XPlane(x0=-n/2, boundary_type='reflective'), openmc.XPlane(x0=n/2, boundary_type='reflective')
ymin, ymax = openmc.YPlane(y0=-n/2, boundary_type='reflective'), openmc.YPlane(y0=n/2, boundary_type='reflective')
...
xy_xs = +xmin & -xmax & +ymin & -ymax

cell_str1 = openmc.Cell(name='eurofer wall LL', region=xy_xs & +z_str1_l & -z_br1_l, fill=eurofer)
if hom == True: 
    cell_br1 = openmc.Cell(name='breeder blanket L', region=xy_xs & +z_br1_l & -z_str2_l, fill=homogeneous_blanket)
    cell_br1 = [cell_br1]
else: 
    if fertile_bulk_density_kgm3 == 0: 
        pass
    else: 
        print("PACKING INNER")
        start_time = time.time()
        br1_centers = openmc.model.pack_spheres(radius=BISO_RADIUS, region=xy_xs & +z_br1_l & -z_str2_l,
                                                pf=vf_biso, seed=1, initial_pf=0.1)
        br1_kernel_cells = []
        ...
        swiss_cheese1_region = xy_xs & +z_br1_l & -z_str2_l
        for biso in br1_coating_surfaces:
            swiss_cheese1_region &= +biso 
...
cell_str2 = openmc.Cell(name='eurofer wall LR', region=xy_xs & +z_str2_l & -z_fw1_l, fill=eurofer)
cell_fw1 = openmc.Cell(name='first wall L', region=xy_xs & +z_fw1_l & -z_p_l, fill=firstwall)
cell_p = openmc.Cell(name='plasma chamber', region=xy_xs & +z_p_l & -z_fw2_l)
cell_fw2 = openmc.Cell(name='first wall R', region=xy_xs & +z_fw2_l & -z_str3_l, fill=firstwall)
cell_str3 = openmc.Cell(name='eurofer wall RL', region=xy_xs & +z_str3_l & -z_br2_l, fill=eurofer)
if hom==True:
    cell_br2 = openmc.Cell(name='breeder blanket R', region=xy_xs & +z_br2_l & -z_str4_l, fill=homogeneous_blanket)
    ...
    if fertile_bulk_density_kgm3 == 0: 
        ...
        print("PACKING OUTER")
        start_time = time.time()
        br2_centers = openmc.model.pack_spheres(radius=BISO_RADIUS, region=xy_xs & +z_br2_l & -z_str4_l,
                                                pf=vf_biso, seed=1, initial_pf=0.1)
        br2_kernel_cells = []
        ...
        swiss_cheese2_region = xy_xs & +z_br2_l & -z_str4_l
...
cell_str4 = openmc.Cell(name='eurofer wall RR', region=xy_xs & +z_str4_l & -z_end, fill=eurofer)
