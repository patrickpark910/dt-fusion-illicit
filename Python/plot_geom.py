...
def add_radial_fill(ax, R, Z, R0, color, n_grid=600):
    """Fill the closed contour (R, Z) with a radial gradient: `color`
    concentrated at the geometric center (R0, 0), fading to white at the
    contour boundary. The fade follows the true boundary shape (the contour
    radius at each angle), so it reaches white right at the edge."""
    # Boundary radius as a function of angle about the center (R0, 0).
    phi_b = np.arctan2(Z, R - R0)
    r_b   = np.hypot(R - R0, Z)
    order = np.argsort(phi_b)
    phi_b, r_b = phi_b[order], r_b[order]

    # Pixel grid over the contour's bounding box.
    r_lo, r_hi = R.min(), R.max()
    z_lo, z_hi = Z.min(), Z.max()
    RG, ZG = np.meshgrid(np.linspace(r_lo, r_hi, n_grid),
                         np.linspace(z_lo, z_hi, n_grid))

    phi     = np.arctan2(ZG, RG - R0)
    rad     = np.hypot(RG - R0, ZG)
    rad_bnd = np.interp(phi, phi_b, r_b, period=2 * np.pi)
    rho     = np.clip(rad / rad_bnd, 0, 1)          # 0 at center, 1 at boundary

    base  = np.array(mcolors.to_rgb(color))
    white = np.ones(3)
    img   = (base * (1 - rho[..., None])) + (white * rho[..., None])

    im = ax.imshow(img, extent=[r_lo, r_hi, z_lo, z_hi], origin='lower',
                   aspect='auto', zorder=0, interpolation='bilinear')
    im.set_clip_path(Path(np.column_stack([R, Z])), ax.transData)
...
def draw_flibe(ax):
    ...
    col = [C_FW, C_BE, C_STRUCTURE, C_FLIBE,
           C_STRUCTURE, C_FLIBE, C_STRUCTURE]

    add_radial_fill(ax, *c[0], R0, C_PLASMA)
...
def draw_dcll(ax):
    ...
    ob_col = [C_STRUCTURE, C_DCLL, C_STRUCTURE, C_STRUCTURE, C_SHIELD]
...
if __name__ == '__main__':
    ...
    #     # Patch(fc=C,  ec=EC, lw=LW, label='Breeding'),
