...
def main():
    ...
    # Since yours default to True, we use 'store_false' to create "disable" flags.
    ...
    if run_type == 'plot':
        ...
        for blanket in blankets:  # make this match class name
            ...
            if os.path.exists(f"{current_run.path}/volume_1.h5"):
                print(f"{C.RED}Warning.{C.END} File {current_run.path}/volume_1.h5 already exists, so this OpenMC volume calculation will be skipped...")
            elif current_run.run_openmc:
                current_run.volumes()
        ...
        for blanket in blankets:  # ['FLiBe', 'DCLL', 'HCPB', 'ARC', 'ARCB']  # make each blanket str match class name
            for fertile_isotope in isotopes:
                ...
                print(f"{C.YELLOW}Comment.{C.END} Collating tallies for {blanket} / {current_run.temp_k} K / {current_run.breeder_enrich}% Li6 / {fertile_isotope} across all fertile loadings...")
...
def collate_tallies(blanket, fertile_isotope, breeder_enrich,temp_k, vol_m3):
    ...
    for folder in tally_folders:
        ...
        try:
            ...
            print(f"{C.RED}Warning.{C.END} File 'tallies_summary.csv' not found in {folder}, skipping...")
            continue

        try:
            df_ng = pd.read_csv(tally_ng)
        except FileNotFoundError:
            print(f"{C.RED}Warning.{C.END} File '{fertile_isotope}_n-gamma_Ebins.csv' not found in {folder}, skipping...")
            continue

        try:
            df_flux = pd.read_csv(tally_flux)
        except FileNotFoundError:
            print(f"{C.RED}Warning.{C.END} File '{fertile_isotope}_flux_Ebins.csv' not found in {folder}, skipping...")
            continue
